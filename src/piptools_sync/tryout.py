...
# Local modules
from . import MAPPING_FILE, ROOT_DIR, logger
...
def generate_db(force=0):
    ...
    if not MAPPING_FILE.exists() or force == 1 or MAPPING_FILE.stat().st_size < 5:
        ...
        logger.info("mapping expired... Generating a new mapping")
        mapping = generate_file()
    else:
        logger.info("Reusing mapping")
...
def find_requirements_file():
    ...
    if final == 1:
        ...
        print(f"Found requirement file: {filename}")
        return result
    elif final == -1:
        raise FileNotFoundError("Requirement file generated by piptools Not found...")
...
def main() -> int:
    ...
    if mismatch > 0:
        ...
        print("Success! - pre-commit is in sync with piptools")
        return 0
